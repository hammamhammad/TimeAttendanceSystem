...
public class CompanySettingsResolver : ICompanySettingsResolver
{
    private readonly IApplicationDbContext _context;
    private readonly IMemoryCache _cache;
    private static readonly TimeSpan CacheDuration = TimeSpan.FromMinutes(5);

    public CompanySettingsResolver(IApplicationDbContext context, IMemoryCache cache)
    {
        _context = context;
        _cache = cache;
    }
    ...
    {
        var companySettings = await GetCompanySettingsCachedAsync(ct);
        var resolved = MapFromCompanySettings(companySettings);
    ...
    private async Task<Domain.Company.CompanySettings?> GetCompanySettingsCachedAsync(CancellationToken ct)
    {
        const string cacheKey = "company-settings";

        if (_cache.TryGetValue<Domain.Company.CompanySettings>(cacheKey, out var cached))
            return cached;
    ...
    private static ResolvedSettingsDto MapFromCompanySettings(Domain.Company.CompanySettings? settings)
    {
        ...
        // Phase 2: timezone + dedup
        dto.DefaultTimeZoneId = string.IsNullOrWhiteSpace(settings.DefaultTimeZoneId) ? "UTC" : settings.DefaultTimeZoneId;
        dto.AttendanceDuplicateSuppressionSeconds = settings.AttendanceDuplicateSuppressionSeconds;
