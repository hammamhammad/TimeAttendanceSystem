...
namespace TecAxle.Hrms.Domain.Company;
...
public class CompanySettings : BaseEntity
{
    ...
    /// <summary>
    /// Phase 2: Company-wide default timezone (IANA id like "Asia/Riyadh" or Windows id like "Arab Standard Time").
    /// Used by <c>ITimezoneService</c> when a branch has no <c>TimeZone</c> set. Fallback: "UTC".
    /// </summary>
    public string DefaultTimeZoneId { get; set; } = "UTC";

    /// <summary>
    /// Phase 2: Seconds window within which a second identical punch (same employee, same transaction type,
    /// same manual/device source) is suppressed as a duplicate. Set to 0 to disable suppression.
    /// </summary>
    ...
    // Phase 6 (v14.6): Removed `AutoCheckOutEnabled` + `AutoCheckOutTime`.
    // The company-wide clockout model was a Phase 0 legacy that was superseded by
    // ShiftDrivenAutoCheckOutJob in Phase 3 (v14.3) and write-frozen in Phase 4.
    // Phase 6 removes them from the entity/DTOs/UI and drops the columns.
