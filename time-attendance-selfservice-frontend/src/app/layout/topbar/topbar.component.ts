...
export class TopbarComponent {
  ...
  // Display name - show fullName (localized) if available, otherwise username
  displayName = computed(() => {
    const user = this.currentUser();
    if (!user) return '';

    // For Arabic locale, prefer Arabic name if available
    if (this.currentLocale() === 'ar' && user.fullNameAr) {
      return user.fullNameAr;
    }

    // Return English fullName if available, otherwise username
