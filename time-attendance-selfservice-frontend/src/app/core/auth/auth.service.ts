...
import { User, LoginRequest, LoginResponse, RefreshTokenRequest, RefreshTokenResponse, ChangePasswordRequest } from '../../shared/models/user.model';
...
export class AuthService {
  ...
  /** localStorage key for must change password flag */
  ...
  /** Computed signal indicating if current user is a manager (has direct reports) */
  public isManager = computed(() => this.currentUser()?.roles?.includes('Manager') || this.currentUser()?.isManager || false);
  ...
  /**
   * Checks if the user must change their password on first login.
  ...
  /**
   * Clears the must change password flag after successful password change.
   */
   ...
   * Used for forced password change on first login.
   *
   * @param request - Change password request containing current and new passwords
   * @returns Observable<void>
   */
  changePassword(request: ChangePasswordRequest): Observable<void> {
    return this.http.post<void>(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.auth.changePassword}`, request)
      .pipe(
        tap(() => {
