...
- **MediatR Pipeline Behaviors**: `[RequiresModule]` attribute + `ModuleEntitlementBehavior` for automatic module enforcement; `[RequiresLimit]` + `UsageLimitBehavior` for usage limit enforcement. **178 commands/queries decorated** across all non-Core modules with two-tier access: commands fully blocked, queries allow read-only historical access via `AllowReadWhenDisabled = true`. **`[RequiresLimit]` is wired on `CreateEmployeeCommand` (`MaxEmployees`) and `CreateBranchCommand` (`MaxBranches`)** — first concrete uses of the usage-limit pipeline.
- **HTTP-layer entitlement filter** (`RequiresModuleEndpointAttribute` + `AllowModuleReadOnlyAttribute` in `src/Api/.../Filters/`): mirrors the MediatR behavior contract for controllers that bypass MediatR (inject `IApplicationDbContext` directly) or mix modules per method. Same `IEntitlementService` source of truth, same SystemAdmin / no-tenant bypass, returns `403` with `{ statusCode, message, traceId }`. **23 controllers** decorated at class level (Recruitment 7, Onboarding 3, Performance 6, Offboarding 2, EmployeeLifecycle 2 — including the 29-endpoint EmployeeDetailsController, Payroll 2 — including the 20-endpoint PayrollSettingsController, plus MobileScheduleController). `[AllowModuleReadOnly]` on every list/detail GET preserves historical read access after a downgrade. **`PortalController` carries 63 per-method decorations** across 17 modules (Core methods like dashboards / profile / team / approvals deliberately unenforced). **`ReportsController` consolidated** onto the same attribute (replacing the previous manual `IEntitlementService` calls per endpoint).
...
- **Module Entitlement Enforcement (two paths, one source of truth)**:
  - **MediatR path (existing)**: 178 non-Core commands/queries decorated with `[RequiresModule(SystemModule.X)]`. Commands fully blocked when module disabled; queries allow historical data access via `AllowReadWhenDisabled = true`. Enforced by `ModuleEntitlementBehavior`.
  - **HTTP-filter path (added v13.2)**: `[RequiresModuleEndpoint(SystemModule.X)]` (+ optional `[AllowModuleReadOnly]` for safe-method historical access) decorates the 23 controllers that use `IApplicationDbContext` directly and therefore bypass MediatR. `PortalController` carries 63 per-method decorations across 17 modules. Both paths bypass for `SystemAdmin` and for unresolved tenant context (platform admin / pre-auth).
  - **`Result<T>` failure resolution fix (v13.2)**: both `ModuleEntitlementBehavior` and `UsageLimitBehavior` now correctly construct `Result<T>.Failure(...)` via `MakeGenericMethod` instead of falling through to `throw new UnauthorizedAccessException(...)` — the previous reflection lookup couldn't find the generic factory on the non-generic base class. The fix means any `Result<T>`-returning command hitting a disabled module / exceeded limit now returns a clean `Result.Failure` at the command boundary instead of a thrown exception mapped to 401.
- **Reports Module Checks**: `ReportsController` is decorated per-action with `[RequiresModuleEndpoint(SystemModule.X)] + [AllowModuleReadOnly]` — attendance→TimeAttendance, leave→LeaveManagement, payroll→Payroll, contracts→EmployeeLifecycle, documents→Documents, certifications→Training, compliance→EmployeeLifecycle. SystemAdmin bypasses. (Replaces the previous in-method `IEntitlementService.IsModuleEnabledAsync()` checks.)
- **Subscription cache invalidation**: every subscription-mutating handler must call `IEntitlementService.InvalidateCache(tenantId)` after `SaveChangesAsync`. `AssignPlanCommandHandler`, `ChangePlanCommandHandler`, `CancelSubscriptionCommandHandler` already do. `UpdateSubscriptionPlanCommandHandler` (v13.2) now invalidates cache for **every tenant on the updated plan** — without this, plan-level module changes were invisible for up to 5 minutes.
- **First `RequiresLimit` usages (v13.2)**: `CreateEmployeeCommand` → `LimitType.MaxEmployees`, `CreateBranchCommand` → `LimitType.MaxBranches`. `EntitlementService.GetCurrentUsageAsync` already counts active rows correctly for these limits.
...
- **TenantSettings Entity**: Centralized strongly-typed operational settings per tenant. As of v13.5 covers **10 categories** (General, Attendance, Leave, Payroll, Approval, Notification, Mobile, Security, Business-Rule Thresholds, **Lifecycle Automation**). v13.3 added 15 fields for password / login-lockout / probation / upload caps / vacation & shift validation / excuse windows / reminder-day CSVs. v13.4 added 9 more: `AttendanceCorrectionMaxRetroactiveDays` (default 30), `DocumentExpiryAlertDaysCsv` (`"30,15,7"`), `AssetWarrantyExpiryAlertDaysCsv` (`"30,15,7,1"`), `AssetOverdueReturnAlertDaysCsv` (`"1,3,7,14,30"`), `TrainingSessionReminderDaysCsv` (`"7,3,1"`), `SuccessionPlanReminderDaysCsv` (`"30,7,1"`), `TimesheetSubmissionReminderDaysBefore` (2), `GrievanceSlaAlertDaysCsv` (`"3,1"`), `NotificationRecipientRolesCsv` (`"HRManager,SystemAdmin"`). v13.5 added 16 lifecycle-automation flags: `LifecycleAutomationEnabled` (master kill-switch, default true), `AutoCreateOnboardingOnOfferAcceptance` (true), `DefaultOnboardingTemplateId`, `CreateEmployeeInactiveAtOfferAcceptance` (false, opt-in pre-hire gate), `AutoActivateEmployeeOnOnboardingComplete` (false), `OnboardingCompletionRequiresAllRequiredTasks` (true), `OnboardingCompletionRequiresAllRequiredDocuments` (false), `AutoCreateTerminationOnResignationApproved` (false), `AutoCreateClearanceOnTermination` (true), `DefaultClearanceTemplateId`, `AutoSuspendEmployeeOnTerminationCreated` (true), `RequireClearanceCompleteBeforeFinalSettlement` (false), `AutoEnableFinalSettlementCalcOnClearanceComplete` (false), `AutoDeactivateEmployeeOnFinalSettlementPaid` (true), `ContractExpiredAction` (`"AutoMarkExpired"`). Every field's DB default mirrors the pre-refactor hardcoded value — **zero behavior change on deploy**. One row per tenant in `TenantSettings` table.
...
- **LifecycleAutomationController** (v13.5) - Read-only audit queries for the lifecycle-automation trail (`GET /api/v1/lifecycle-automation/audit`, `GET /audit/{id}`, `GET /audit/by-entity`). Used by the Automation History card on detail pages.
...
- **`ITenantPayrollCalendarService`** (v13.3, `TenantPayrollCalendarService`) — convenience wrapper for one-off monthly daily-rate resolution outside the payroll pipeline. Used by `CreateLeaveEncashmentCommandHandler` and `CalculateFinalSettlementCommandHandler` to replace the legacy `/ 30m` hardcode. Looks up the active `PayrollCalendarPolicy` for `(branchId, referenceDate)` and returns the basis days; falls back to `DateTime.DaysInMonth` when no policy is configured.

#### End-of-Service (EOS) Calculation — v13.3
- **`EndOfServicePolicy`** (+ `EndOfServicePolicyTier` + `EndOfServiceResignationDeductionTier`) — tenant-scoped, effective-dated, optional `CountryCode`. Replaces the Saudi-hardcoded tier formula that used to live in `CalculateEndOfServiceCommandHandler`. One Saudi-default policy is auto-seeded per tenant at provisioning time (`TenantProvisioningService`) and on first boot (`SeedData.SeedDefaultEndOfServicePolicyAsync`) — identical to the pre-v13.3 formula (`[0,5y) × 0.5mo/y`, `[5y,∞) × 1.0mo/y`, resignation deductions `<2y=100%`, `[2,5y)=2/3`, `[5,10y)=1/3`, `10y+=0%`).
- **`IEndOfServiceCalculator`** (`EndOfServiceCalculator`) — resolves the applicable policy by effective date + optional country, applies tier multipliers, computes resignation deduction, and persists the applied policy ID + JSON snapshot on `EndOfServiceBenefit.AppliedPolicySnapshotJson` for forensic audit. On missing policy, emits a warning log and falls back to the seeded Saudi formula — never silently zeros.
- **CRUD API**: `GET/POST/PUT/DELETE /api/v1/end-of-service-policies` + `GET /api/v1/end-of-service-policies/{id}` via [EndOfServicePoliciesController](src/Api/TimeAttendanceSystem.Api/Controllers/EndOfServicePoliciesController.cs). Create/Update reject overlapping active policies for the same country; Delete is blocked when any `EndOfServiceBenefit` references the policy. Class-level `[RequiresModuleEndpoint(SystemModule.Offboarding)]` with `[AllowModuleReadOnly]` on the list/detail GETs.

#### Hardcoded-Rules Elimination — v13.3 + v13.4

**v13.3** moved 18 hardcoded business rules into `TenantSettings` + new entities. Highlights beyond EOS:
- **`ISystemUserResolver`** (`SystemUserResolver`) — returns the tenant's `IsSystemUser=true` user ID (preferring `systemadmin` / `tecaxleadmin`) for auto-created records. Replaces the previous `AssignedByUserId = 1` FK risk in `DailyAttendanceGeneratorService`. Cached 5-min per tenant via `IMemoryCache`; throws on unresolvable — no silent fallback to ID 1.
- **`IValidationSettingsProvider`** (`ValidationSettingsProvider`) — scoped cache of tenant thresholds for FluentValidation rule chains. `WarmAsync()` hydrates; `Current` is read synchronously inside rules. Consumed by `CreateEmployeeVacationCommandValidator`, `CreateBulkEmployeeVacationCommandValidator`, `CreateShiftCommandValidator`, `CreateEmployeeExcuseCommandValidator`, and both `CreateAttendanceCorrectionRequestCommandValidator` / `UpdateAttendanceCorrectionRequestCommandValidator`.
- **`LoginLockoutPolicy`** — parses `TenantSettings.LoginLockoutPolicyJson` into an ordered tier list with graceful fallback to the pre-v13.3 `5→15m / 10→1h / 15→24h` ladder.
- **Leave encashment & final settlement** — `CreateLeaveEncashmentCommandHandler:91` and `CalculateFinalSettlementCommandHandler:52` now call `ITenantPayrollCalendarService.GetMonthlyDailyBasisAsync(branchId, refDate)` instead of `/ 30m`.

**v13.4** (this release) closed the remaining 9 hardcoded business rules + consolidated 15 divergent `Role.Name == "HR"` / `"HRManager"` / `"Admin"` checks:
- **`INotificationRecipientResolver`** (`NotificationRecipientResolver`) — resolves notification recipient user IDs from `TenantSettings.NotificationRecipientRolesCsv` (default `"HRManager,SystemAdmin"`). Consumed by every background job that creates `Notification` rows and by `PortalController` / `DisciplinaryActionsController` / `InvestigationsController` — 15 call sites collapsed into one abstraction. Accepts an optional `extraRoles` parameter so legacy callers that needed `"Admin"` (SuccessionPlan/Asset jobs) or `"HR"` (Grievance, Portal, Disciplinary, Investigations) still work while tenants migrate. Case-insensitive role matching; empty CSV falls back to hardcoded default; all matches produce a distinct user-id list (empty is legal and logged, not thrown).
- **Every alert / reminder window is tenant-configurable**: 7 more jobs moved to CSV settings (`DocumentExpiryAlertJob`, `AssetWarrantyExpiryAlertJob`, `OverdueAssetReturnAlertJob`, `TrainingSessionReminderJob`, `SuccessionPlanReviewReminderJob`, `TimesheetSubmissionReminderJob`, `GrievanceSlaAlertJob`). All use the shared `BackgroundJobSettingsHelper.ParseCsvDays(csv, fallback)` parser that rejects malformed entries and dedupes.
- **Attendance-correction retroactive cap** moved from `private const int MaxRetroactiveDays = 30` in 3 files to `TenantSettings.AttendanceCorrectionMaxRetroactiveDays` (default 30).
- **Document "expiring soon" horizon** in `EmployeeDocumentsController` now uses `max(DocumentExpiryAlertDaysCsv)` instead of a fixed 30-day filter.

#### Lifecycle Automation — v13.5

Automates seven HR lifecycle transitions that were previously manual, hardcoded, or broken. Every automation is per-tenant configurable, auditable, idempotent, and backward-compatible by default.

- **7 transitions orchestrated via domain events** (MediatR `INotification`): `OfferAccepted` → onboarding; `OnboardingCompleted` → activate employee; `ResignationApproved` → termination; `TerminationCreated` → clearance + suspend employee (two sub-steps); `ClearanceCompleted` → enable/calculate settlement; `FinalSettlementPaid` → deactivate employee; `ContractExpired` → configured action. Events published in [src/Application/.../Lifecycle/Events/LifecycleEvents.cs](src/Application/TimeAttendanceSystem.Application/Lifecycle/Events/LifecycleEvents.cs).
- **`ILifecycleEventPublisher`** ([abstraction](src/Application/TimeAttendanceSystem.Application/Abstractions/ILifecycleEventPublisher.cs), [impl](src/Infrastructure/TimeAttendanceSystem.Infrastructure/Services/LifecycleEventPublisher.cs)) wraps MediatR `IPublisher` with try/catch + log so a handler failure **never propagates back to the triggering command** — the approval / acceptance / paid-mark always succeeds even if a downstream automation bug throws. The exception lands as a `Failed` row in the audit table instead of an HTTP 500.
- **`LifecycleAutomationAudit` entity** ([Domain/Lifecycle](src/Domain/TimeAttendanceSystem.Domain/Lifecycle/LifecycleAutomationAudit.cs)) — append-only, one row per automation attempt (`Succeeded | Skipped | Failed | DuplicateSuppressed | Disabled | MissingPrerequisite`). Indexes: `(SourceEntityType, SourceEntityId, AutomationType, Status)` for idempotency lookups, `(SourceEntityType, SourceEntityId)` for detail-page timelines. Carries `TargetEntityType`/`TargetEntityId`, `Reason`, `ErrorMessage`, `ContextJson` forensic snapshot.
- **`LifecycleAutomationBase`** ([Application/Lifecycle/Handlers](src/Application/TimeAttendanceSystem.Application/Lifecycle/Handlers/LifecycleAutomationBase.cs)) — shared plumbing every `INotificationHandler<T>` inherits: load settings → master kill-switch check → per-automation flag check → idempotency key lookup → run downstream command → write audit row. Exceptions caught inside, never propagate.
- **7 MediatR commands** (idempotent, each writes side-effects only):
  - `CreateOnboardingProcessFromOfferCommand` — resolves template via explicit `DefaultOnboardingTemplateId` override → dept → branch → IsDefault fallback; fails cleanly with `MissingPrerequisite` audit row if none resolves.
  - `CreateTerminationFromResignationCommand` — fires only on `ResignationStatus.Approved`; dedupes on `ResignationRequestId → TerminationRecord` link.
  - `CreateClearanceFromTemplateCommand` — `DefaultClearanceTemplateId == null` seeds 8 hardcoded default items (IT×2, Finance×2, Admin×2, HR×2) for v13.x parity; `ContextJson` records which path was taken.
  - `ApplyContractExpiredActionCommand` — applies the tenant's `ContractExpiredAction` (`NotifyOnly | AutoMarkExpired | Suspend | Deactivate`). Degrades `Suspend`/`Deactivate` → `AutoMarkExpired` if employee is already inactive.
  - `SuspendEmployeeCommand` / `ActivateEmployeeCommand` / `DeactivateEmployeeCommand` — idempotent employee-state transitions.
- **Two-phase suspend/deactivate model** replaces the v13.x "deactivate at termination" coupling:
  - **At termination**: `Employee.IsSuspended = true` + linked `User.IsActive = false` (login blocked, employee record still visible in reports / payroll / clearance).
  - **At final settlement paid**: `Employee.IsActive = false` (fully off-boarded, hidden from normal operations).
  - With default flags (`AutoSuspendEmployeeOnTerminationCreated=true` + `AutoDeactivateEmployeeOnFinalSettlementPaid=true`), the observable end-state equals v13.x — just with clearer semantics.
- **`Employee` entity** gets 3 new fields: `IsSuspended` (termination marker, cleared at full deactivation), `IsPreHire` (opt-in pre-hire gate marker; only set when `CreateEmployeeInactiveAtOfferAcceptance=true`), `OnboardingCompletedAt` (milestone timestamp, always stamped).
- **16 new `TenantSettings` fields** (all additive, DB defaults preserve v13.x behavior — **zero behavior change on deploy**):
  - `LifecycleAutomationEnabled` (bool, default `true`) — master kill-switch; when false every handler writes a `Disabled` row and short-circuits.
  - `AutoCreateOnboardingOnOfferAcceptance` (bool, default `true`) — matches v13.x hardcoded behavior.
  - `DefaultOnboardingTemplateId` (long?) — explicit override; null = dept/branch/IsDefault fallback.
  - `CreateEmployeeInactiveAtOfferAcceptance` (bool, default `false`) — opt-in pre-hire gate.
  - `AutoActivateEmployeeOnOnboardingComplete` (bool, default `false`) — opt-in; default = milestone-only.
  - `OnboardingCompletionRequiresAllRequiredTasks` (bool, default `true`) — matches v13.x validation.
  - `OnboardingCompletionRequiresAllRequiredDocuments` (bool, default `false`) — new opt-in gate.
  - `AutoCreateTerminationOnResignationApproved` (bool, default `false`) — opt-in.
  - `AutoCreateClearanceOnTermination` (bool, default `true`) — matches v13.x.
  - `DefaultClearanceTemplateId` (long?) — null = seed 8 hardcoded items (v13.x parity).
  - `AutoSuspendEmployeeOnTerminationCreated` (bool, default `true`) — replaces v13.x immediate `IsActive=false`.
  - `RequireClearanceCompleteBeforeFinalSettlement` (bool, default `false`) — opt-in; when true, `CalculateFinalSettlementCommand` rejects until clearance is `Completed`.
  - `AutoEnableFinalSettlementCalcOnClearanceComplete` (bool, default `false`) — opt-in; when true, clearance completion auto-triggers `CalculateFinalSettlementCommand`.
  - `AutoDeactivateEmployeeOnFinalSettlementPaid` (bool, default `true`) — completes the v13.x end-state.
  - `ContractExpiredAction` (string enum, default `"AutoMarkExpired"`) — fixes the silent pre-v13.5 bug where contracts stayed `Active` past `EndDate`. Values: `NotifyOnly | AutoMarkExpired | Suspend | Deactivate`.
- **Duplicate prevention** — two layers: (1) audit-row idempotency key `(SourceEntityType, SourceEntityId, AutomationType, Status=Succeeded)` — handler writes `DuplicateSuppressed` on replay; (2) target-side existence checks (e.g. skip if `OnboardingProcess` exists for employee, if `TerminationRecord` references resignation, if `ClearanceChecklist` references termination).
- **Publisher wiring** — handlers edited to publish events AFTER `SaveChangesAsync`: [ApproveResignationCommandHandler](src/Application/TimeAttendanceSystem.Application/Resignations/Commands/ApproveResignation/ApproveResignationCommandHandler.cs), [CreateTerminationRecordCommandHandler](src/Application/TimeAttendanceSystem.Application/Terminations/Commands/CreateTerminationRecord/CreateTerminationRecordCommandHandler.cs) (inline clearance + deactivation removed — now driven by handler), [MarkFinalSettlementPaidCommandHandler](src/Application/TimeAttendanceSystem.Application/FinalSettlements/Commands/MarkFinalSettlementPaid/MarkFinalSettlementPaidCommandHandler.cs), [CalculateFinalSettlementCommandHandler](src/Application/TimeAttendanceSystem.Application/FinalSettlements/Commands/CalculateFinalSettlement/CalculateFinalSettlementCommandHandler.cs) (now respects `RequireClearanceCompleteBeforeFinalSettlement`), [OfferLettersController.AcceptOffer](src/Api/TimeAttendanceSystem.Api/Controllers/OfferLettersController.cs) (inline onboarding creation replaced with event publish; supports `IsPreHire` mode), [OnboardingProcessesController.Complete](src/Api/TimeAttendanceSystem.Api/Controllers/OnboardingProcessesController.cs) (new docs gate + event publish), [ClearanceController.CompleteItem](src/Api/TimeAttendanceSystem.Api/Controllers/ClearanceController.cs) (publishes on flip-to-Completed only).
- **`ContractExpiryAlertJob` extended** — still sends pre-expiry alerts in configured windows (`ContractExpiryAlertDaysCsv`), additionally publishes `ContractExpiredEvent` for every contract with `EndDate <= today` and `Status == Active`. Handler applies the configured action, which closes the silent "Active past end date" data-integrity bug by default.
- **Audit API** — new [LifecycleAutomationController](src/Api/TimeAttendanceSystem.Api/Controllers/LifecycleAutomationController.cs) with three read-only endpoints: `GET /api/v1/lifecycle-automation/audit` (paginated, filterable), `GET /audit/{id}` (single with `ContextJson`), `GET /audit/by-entity?entityType=...&entityId=...` (used by Automation History detail-page card).
- **Workflow approval gates respected** — `ApproveResignationCommandHandler` still requires `Status == Pending` before publishing; `MarkFinalSettlementPaidCommandHandler` still requires `Status == Approved`. Lifecycle handlers trust those contracts. Rejected / cancelled / withdrawn resignation paths publish nothing.
- **EF migration** `20260416072853_AddLifecycleAutomationV13_5.cs` — additive only: 3 columns on `Employees`, 16 columns on `TenantSettings`, 1 new table `LifecycleAutomationAudits` with 2 indexes. DB defaults match every hardcoded v13.x path bit-for-bit. Zero backfill, zero-downtime deploy-safe.
- **Frontend** (admin portal) — [lifecycle-automation.model.ts](time-attendance-frontend/src/app/shared/models/lifecycle-automation.model.ts) + [lifecycle-automation.service.ts](time-attendance-frontend/src/app/core/services/lifecycle-automation.service.ts) + reusable [AutomationHistoryCardComponent](time-attendance-frontend/src/app/shared/components/automation-history-card/automation-history-card.component.ts) ready to drop onto any detail page (`entityType` + `entityId` inputs). Bilingual `lifecycle_automation.*` translations added to `en.json` + `ar.json`. Tenant-config UI wiring for the 16 new flags is a deferred follow-up — the backend `GET/PUT /api/v1/tenant-configuration` already accepts them via the existing `TenantSettings` shape.

See `LIFECYCLE_AUTOMATION_FIX.md` for per-rule coverage, scenario mapping, and remaining risks.
...
- `ContractExpiryAlertJob` - Daily. Two responsibilities: (1) alert HR for contracts expiring in configured pre-windows (`ContractExpiryAlertDaysCsv`, default "30,15,7"); (2) **v13.5**: publish `ContractExpiredEvent` for every contract with `EndDate <= today` + `Status == Active`, so the lifecycle handler applies the tenant's configured `ContractExpiredAction` (NotifyOnly / AutoMarkExpired / Suspend / Deactivate). Fixes the pre-v13.5 silent "Active past end date" data-integrity bug.
...
- MediatR Behaviors: `src/Application/TecAxle.Hrms.Application/Common/Behaviors/` (`RequiresModuleAttribute` with `AllowReadWhenDisabled`, `ModuleEntitlementBehavior`, `RequiresLimitAttribute`, `UsageLimitBehavior`). Both behaviors resolve `Result<T>.Failure` via `MakeGenericMethod` on the non-generic `Result.Failure<T>` factory.
- HTTP entitlement filter: `src/Api/TecAxle.Hrms.Api/Filters/RequiresModuleEndpointAttribute.cs` (contains both `RequiresModuleEndpointAttribute : IAsyncAuthorizationFilter` and the `AllowModuleReadOnlyAttribute` marker).
...
- **Entitlement Tests**: `tests/TecAxle.Hrms.Entitlement.Tests/` — xUnit + FluentAssertions + Moq + `Microsoft.AspNetCore.Mvc.Testing`. **79 tests total, all green** (run ~6 s):
  - Filter unit tests (`RequiresModuleEndpointAttributeTests`) — every branch of the HTTP filter (SystemAdmin bypass, no-tenant bypass, enabled/disabled, AllowModuleReadOnly safe-method gating, 403 body shape).
  - Behavior unit tests (`UsageLimitBehaviorTests`) — limit-reached → `Result.Failure`, under-limit → pass, SystemAdmin bypass, attribute-presence on `CreateEmployeeCommand` / `CreateBranchCommand`.
  - Reflection regression (`ControllerCoverageTests`) — pins the 23 controllers that must carry `[RequiresModuleEndpoint]`; fails fast if any loses the attribute.
  - End-to-end HTTP (`EndToEnd/`) — `WebApplicationFactory<Program>` boots the real app with `IEntitlementService` → `FakeEntitlementService`, `ITenantConnectionResolver` → stub (avoids real master DB), and a header-driven `TestAuthHandler`. Sends real HTTP requests through the real middleware/routing/filter pipeline. Covers all 6 target modules, ReportsController, MobileScheduleController, and PortalController per-method enforcement (Offboarding, EmployeeRelations, Core profile-still-accessible).
  - Run: `cd tests/TecAxle.Hrms.Entitlement.Tests && dotnet test` (the API process must not be running — DLL locks block the build).
- **BusinessRules Tests (v13.3 + v13.4)**: `tests/TecAxle.Hrms.BusinessRules.Tests/` — xUnit + FluentAssertions + `Microsoft.EntityFrameworkCore.InMemory`. **31 tests, all green**:
  - `EndOfServiceCalculatorTests` (8) — Saudi seed identity with pre-v13.3 formula across the 0-2-5-10-20 year boundary cases; resignation-deduction tiers; effective-date policy selection; missing-policy fallback.
  - `LoginLockoutPolicyTests` (5) — JSON parsing, malformed fallback, highest-tier selection.
  - `BackgroundJobSettingsHelperTests` (5) — CSV parse / dedupe / reject invalid / fallback.
  - `NotificationRecipientResolverTests` (6, v13.4) — default-roles behavior; configured-CSV override; extraRoles merge; empty/whitespace fallback; case-insensitive match; empty result on no match.
  - `V13_4_ConfigurationTests` (7, v13.4) — pins every new `TenantSettings` field default bit-for-bit; asserts `ParseCsvDays` correctly parses each new default shape.
  - Run: `cd tests/TecAxle.Hrms.BusinessRules.Tests && dotnet test`.
- **LifecycleAutomation Tests (v13.5)**: `tests/TecAxle.Hrms.LifecycleAutomation.Tests/` — xUnit + FluentAssertions + `Microsoft.EntityFrameworkCore.InMemory` + Moq. **25 tests, all green** (~6 seconds).
  - `TerminationCreatedHandlerTests` (4) — both sub-steps (clearance + suspend) run with defaults; master kill-switch disables both; per-flag disable runs only the enabled sub-step; duplicate event suppressed.
  - `FinalSettlementPaidHandlerTests` (4) — deactivates when enabled; skips when employee already fully inactive; `Disabled` when flag off; dedupe.
  - `ResignationApprovedHandlerTests` (3) — creates termination when enabled; disabled by default; dedupe.
  - `OnboardingCompletedHandlerTests` (3) — milestone-only default stamps `OnboardingCompletedAt` without activating; activate mode flips `IsPreHire`→`IsActive`; kill-switch.
  - `ContractExpiredHandlerTests` (6) — all four action modes (`NotifyOnly`, `AutoMarkExpired`, `Suspend`, `Deactivate`) dispatched correctly; degrades `Deactivate`→`AutoMarkExpired` when employee already inactive; dedupe.
  - `EmployeeStateCommandTests` (5) — `Suspend`/`Activate`/`Deactivate` idempotency; `Suspend` blocks linked `User.IsActive`; `Deactivate` clears `IsSuspended`; `Activate` flips pre-hire + stamps milestone.
  - Shared [TestHarness.cs](tests/TecAxle.Hrms.LifecycleAutomation.Tests/TestHarness.cs): `NewDb()`, `SeedSettings()`, `SeedEmployee()`, `StubUser()`, `CountAudits()`.
  - Run: `cd tests/TecAxle.Hrms.LifecycleAutomation.Tests && dotnet test`. **Full-suite totals after v13.5**: Payroll 27 + Entitlement 79 + BusinessRules 31 + LifecycleAutomation 25 = **162 passing tests**.
- **EOS + v13.3/v13.4 Migrations**: `src/Infrastructure/TimeAttendanceSystem.Infrastructure/Persistence/PostgreSql/Migrations/20260415180127_AddHardcodedBusinessRulesConfigurationV13_3.cs` (v13.3 — 15 `TenantSettings` columns + 3 EOS tables + 2 `EndOfServiceBenefits` columns) and `20260415184533_AddBusinessRulesConfigV13_4.cs` (v13.4 — 9 more `TenantSettings` columns). Both are additive-only; existing tenants get the exact pre-refactor values via DB `DEFAULT` clauses — zero data loss.
- **v13.5 Migration**: `20260416072853_AddLifecycleAutomationV13_5.cs` — additive only: 3 `Employees` columns (`IsSuspended`, `IsPreHire`, `OnboardingCompletedAt`), 16 `TenantSettings` columns, 1 new table `LifecycleAutomationAudits` with 2 indexes. DB defaults match every hardcoded v13.x path bit-for-bit (e.g. `LifecycleAutomationEnabled=true`, `AutoCreateClearanceOnTermination=true`, `ContractExpiredAction='AutoMarkExpired'`). Zero backfill. **Note**: migration applies to tenant DBs, not master — existing tenant DBs need the migration applied manually (or wait for next provisioning).
...
- **Tenant Configuration — Security tab** (v13.3 + v13.4): `time-attendance-frontend/src/app/pages/settings/tenant-configuration/security-settings/security-settings.component.ts` gains a **"Business Rule Thresholds"** section exposing all 24 new `TenantSettings` fields added by v13.3 + v13.4 (password min length, login lockout JSON, 7 alert/reminder CSVs, probation/upload/vacation/shift caps, excuse windows, OT config future cap, notification recipient roles CSV, attendance-correction retroactive cap). `tenant-configuration.models.ts` DTO extended — all fields optional so legacy clients don't break.
- **End-of-Service Policies page (deferred)**: backend CRUD at `/api/v1/end-of-service-policies` is fully functional; HR admins can manage policies via Swagger in Dev while the rich UI ships in a follow-up PR. Saudi-default policy is auto-seeded per tenant, so existing tenants need no action.
...
**Version**: 13.6 — Workflow Routing Hardening. Fixes 12 latent defects in the approval-workflow engine that could route requests to the wrong person, stall workflows silently, or leave weak audit records. (1) **Department-head routing** no longer uses a heuristic — `ApproverResolver` now resolves via `Department.ManagerEmployeeId` explicitly, then falls back via tenant-configured chain (`WorkflowFallbackApproverUserId` → `WorkflowFallbackApproverRole`). (2) **Role-based routing** is now configurable per-step: `WorkflowStep.RoleAssignmentStrategy` ∈ `{FirstMatch, RoundRobin, LeastPendingApprovals (default), FixedPriority}`. `WorkflowRoleAssignmentCursors` (new table) backs RoundRobin; `UserRole.Priority` (new column) backs FixedPriority. (3) **`BranchManager` approver type** is now implemented — added `Branch.ManagerEmployeeId`. (4) **Delegation** is bounded: `TenantSettings.MaxWorkflowDelegationDepth` (default 2) + cycle detection via `DelegatedFromExecutionId` walk; both delegator and delegate receive notifications. (5) **Timeout/escalation audit** no longer uses string "0" as actor — resolved via existing `ISystemUserResolver`; every system action writes a row to `WorkflowSystemActionAudits` (new append-only table) with `ActionType ∈ {Timeout, Escalation, AutoApprove, AutoReject, Expire, FallbackRouting}` and forensic `DetailsJson`. (6) **Validation steps** now run real rules: new `IWorkflowValidationRule` abstraction + `IWorkflowValidationRuleRegistry`; a step with an unset or unregistered `ValidationRuleCode` fails closed (AutoReject) — no more silent passes. Ships with reference rule `vacation_balance`. (7) **Workflow-definition versioning**: `WorkflowInstance.DefinitionSnapshotJson` + `DefinitionVersion` are populated at instance creation. Admin edits to the live definition no longer affect running instances. (8) **New non-final "Return for Correction" action**: `ApprovalAction.ReturnedForCorrection` + `WorkflowStatus.ReturnedForCorrection` (non-terminal). Per-step opt-in via `WorkflowStep.AllowReturnForCorrection`. Requester resubmits via new `POST /api/v1/approvals/{id}/resubmit` with `ResubmissionCount` capped by `TenantSettings.MaxWorkflowResubmissions` (default 3). Publishes `RequestReturnedForCorrectionEvent` (MediatR INotification) for source modules to listen. (9) **Fail-loud routing**: when primary approver resolution AND fallback chain both miss, workflow transitions to new `WorkflowStatus.FailedRouting` (terminal) and notifies `NotificationRecipientRolesCsv` recipients — no more silent stalls. (10) **4 legacy direct-status approval handlers** (`ApproveEmployeeExcuse`, `ApproveRemoteWorkRequest`, `ApproveAttendanceCorrectionRequest`, `ToggleEmployeeVacationStatus`) tagged `[Obsolete]` for v14 removal; new `IWorkflowStarter` service consolidates the workflow-kickoff contract for future modules. **New API endpoints**: `POST /api/v1/approvals/{id}/return-for-correction`, `POST /api/v1/approvals/{id}/resubmit`, `GET /api/v1/workflows/validation-rules`, `GET /api/v1/workflows/system-actions`, `GET /api/v1/workflows/role-assignment-stats`. **Data model**: new columns on `Branches` (ManagerEmployeeId), `UserRoles` (Priority), `TenantSettings` (4 fields), `WorkflowSteps` (4 fields: RoleAssignmentStrategy, AllowReturnForCorrection, ValidationRuleCode, ValidationConfigJson), `WorkflowInstances` (5 fields: DefinitionVersion, DefinitionSnapshotJson, ReturnedAtUtc, ReturnedByUserId, ResubmissionCount), `WorkflowStepExecutions` (ValidationDetailsJson). Two new tables (`WorkflowRoleAssignmentCursors`, `WorkflowSystemActionAudits`). **Migration** `20260416093000_WorkflowRoutingHardeningV13_6.cs` is additive — every new column carries a default mirroring pre-v13.6 behavior; includes PostgreSQL backfill SQL that populates `DefinitionSnapshotJson` for every non-terminal instance. Zero data loss, zero behavior change on deploy for tenants that don't flip a flag. **Frontend**: updated `workflow.model.ts` in both admin and selfservice with new types/fields; `ApprovalsService` / `PortalService` gain `returnForCorrection()` + `resubmit()`; `WorkflowsService` gains `getValidationRules()` / `getSystemActions()` / `getRoleAssignmentStats()`; bilingual `workflow_routing.*` + `approvals_v136.*` + `portal.return_for_correction*` i18n keys added to both EN and AR. Admin-UI component wiring (Return / Resubmit buttons on pending-approvals modal, step-config form fields, system-actions browser page, role-assignment stats panel, branch manager picker, tenant-config workflow-routing section) is documented in `WORKFLOW_ROUTING_HARDENING_FIX.md` as the immediate frontend follow-up — endpoints and models are ready. **Tests**: new `tests/TecAxle.Hrms.Workflow.Tests/` project with 18 passing tests covering primary+fallback department-head routing, LeastPendingApprovals, FixedPriority, RoundRobin cursor advance, BranchManager resolution, empty-role-pool fallback, snapshot round-trip, non-terminal `ReturnedForCorrection` status, resubmission counter, validation-rule registry lookup (case-insensitive + fail-closed). Full suite now **180 tests across 5 projects, all green** (Payroll 27, Entitlement 79, BusinessRules 31, LifecycleAutomation 25, Workflow 18). Both frontends type-check clean. See `WORKFLOW_ROUTING_HARDENING_FIX.md` for the full forensic design, per-defect resolution table, and remaining risks. Previous: v13.5 — Lifecycle Automation. Automates seven HR lifecycle transitions that were previously manual, hardcoded, or outright broken: (1) **Offer Accepted → Onboarding** — previously inline + hardcoded in `OfferLettersController.AcceptOffer`; now event-driven via `OfferAcceptedEvent` → `CreateOnboardingProcessFromOfferCommand`, respects `DefaultOnboardingTemplateId` + dept/branch/IsDefault fallback, fails cleanly with `MissingPrerequisite` audit row when no template resolves. (2) **Onboarding Completed → Activate** — previously missing; now `OnboardingCompletedEvent` → optional `ActivateEmployeeCommand` (milestone-only by default, opt-in activation flips `IsPreHire`→`IsActive`). (3) **Resignation Approved → Termination** — previously missing (approval only flipped `EmploymentStatus=Resigned`); now opt-in via `AutoCreateTerminationOnResignationApproved`. (4) **Termination Created → Clearance + Suspend** — previously hardcoded 8 items inline in handler; now event-driven with `DefaultClearanceTemplateId` override (null falls back to v13.x 8-item default). Termination also suspends employee (`IsSuspended=true` + linked `User.IsActive=false`) instead of fully deactivating. (5) **Clearance Completed → Enable Settlement** — previously missing; now optional `RequireClearanceCompleteBeforeFinalSettlement` gate + opt-in auto-calc. (6) **Final Settlement Paid → Deactivate** — previously missing (deactivation was coupled to termination creation); new two-phase model: termination suspends, final-settlement-paid deactivates. Defaults preserve v13.x end-state. (7) **Contract Expired → Action** — previously alert-only; `ContractExpiryAlertJob` now publishes `ContractExpiredEvent` for contracts past `EndDate`; handler applies configured `ContractExpiredAction` (NotifyOnly/AutoMarkExpired/Suspend/Deactivate). Default `AutoMarkExpired` fixes the silent "Active past end date" data-integrity bug. **Infrastructure**: new `ILifecycleEventPublisher` wraps MediatR `IPublisher` with try/catch + log — a lifecycle-handler failure never propagates back to the triggering command (approval / acceptance / paid-mark always succeeds, failures land as `Failed` rows in `LifecycleAutomationAudit`, not HTTP 500s). New append-only `LifecycleAutomationAudit` entity captures every attempt (`Succeeded | Skipped | Failed | DuplicateSuppressed | Disabled | MissingPrerequisite`) with source/target entity ids, reason, error, `ContextJson`. 7 `INotification` events (`OfferAccepted`/`OnboardingCompleted`/`ResignationApproved`/`TerminationCreated`/`ClearanceCompleted`/`FinalSettlementPaid`/`ContractExpired`), 7 idempotent MediatR commands, 7 handlers sharing `LifecycleAutomationBase` (settings load → kill-switch → flag check → idempotency key → run → audit). **Two-phase suspend/deactivate** replaces v13.x "deactivate at termination" coupling: 3 new `Employee` fields (`IsSuspended`, `IsPreHire`, `OnboardingCompletedAt`). **Configuration**: 16 new `TenantSettings` fields with DB defaults mirroring v13.x behavior — `LifecycleAutomationEnabled` master kill-switch (default `true`), per-transition enable flags, template/policy overrides. **Idempotency**: audit-row key `(Source, Id, Type, Status=Succeeded)` + target-side existence checks (skip if onboarding/termination/clearance already exists). **Workflow respect**: approval gates in existing handlers are unchanged; lifecycle handlers trust those contracts. **API**: new read-only `LifecycleAutomationController` (`GET /api/v1/lifecycle-automation/audit`, `GET /audit/{id}`, `GET /audit/by-entity`) for HR to see what fired, failed, or was suppressed. **Frontend**: `lifecycle-automation.model.ts` + service + reusable `AutomationHistoryCardComponent` (drop onto any detail page with `entityType` + `entityId`) + bilingual `lifecycle_automation.*` translations. Wiring onto the seven specific view pages + tenant-config settings sub-tab is a deferred UI follow-up (backend endpoints ready). **Migration**: `20260416072853_AddLifecycleAutomationV13_5.cs` — additive only (3 `Employees` columns, 16 `TenantSettings` columns, new `LifecycleAutomationAudits` table + 2 indexes). Zero behavior change on deploy until a tenant flips a flag. **Note**: migration targets tenant DBs — existing tenant DBs need the migration applied manually. **Tests**: new `tests/TecAxle.Hrms.LifecycleAutomation.Tests/` project with 25 passing tests covering handler behavior, idempotency, kill-switch, per-flag disable, and employee-state commands. Full suite now **162 tests across 4 projects, all green** (Payroll 27, Entitlement 79, BusinessRules 31, LifecycleAutomation 25). Existing handlers/controllers modified to publish events after `SaveChangesAsync`: `ApproveResignationCommandHandler`, `CreateTerminationRecordCommandHandler` (inline clearance + deactivation removed, now handler-driven), `MarkFinalSettlementPaidCommandHandler`, `CalculateFinalSettlementCommandHandler` (respects clearance-complete gate), `OfferLettersController.AcceptOffer`, `OnboardingProcessesController.Complete` (new required-docs gate), `ClearanceController.CompleteItem` (publishes on flip-to-Completed only). See `LIFECYCLE_AUTOMATION_FIX.md`. Previous: v13.4 — Hardcoded Business Rules Remaining Fix. Closes the 9 residual hardcoded business rules identified by the post-v13.3 audit (`HARDCODED_RULES_AUDIT.md`) and consolidates 15 divergent `Role.Name == "HR" / "HRManager" / "Admin"` notification-recipient checks into a single abstraction. (1) **9 new `TenantSettings` fields**: `AttendanceCorrectionMaxRetroactiveDays` (30), `DocumentExpiryAlertDaysCsv` (`"30,15,7"`), `AssetWarrantyExpiryAlertDaysCsv` (`"30,15,7,1"`), `AssetOverdueReturnAlertDaysCsv` (`"1,3,7,14,30"`), `TrainingSessionReminderDaysCsv` (`"7,3,1"`), `SuccessionPlanReminderDaysCsv` (`"30,7,1"`), `TimesheetSubmissionReminderDaysBefore` (2), `GrievanceSlaAlertDaysCsv` (`"3,1"`), `NotificationRecipientRolesCsv` (`"HRManager,SystemAdmin"`) — all DB defaults mirror pre-v13.4 hardcoded values bit-for-bit. (2) **`INotificationRecipientResolver`** new abstraction + `NotificationRecipientResolver` impl collapses 15 `UserRoles.Where(ur => ur.Role.Name == "…")` call sites across 9 background jobs (`ContractExpiryAlertJob`, `VisaExpiryAlertJob`, `DocumentExpiryAlertJob`, `CertificationExpiryAlertJob`, `LoanRepaymentReminderJob`, `OnboardingTaskOverdueJob`, `PIPExpiryCheckJob`, `AssetWarrantyExpiryAlertJob`, `OverdueAssetReturnAlertJob`, `SuccessionPlanReviewReminderJob`, `GrievanceSlaAlertJob`) + 3 controllers (`PortalController` ×2, `DisciplinaryActionsController`, `InvestigationsController`). Legacy callers pass `extraRoles` (e.g. `"HR"`, `"Admin"`) to preserve backward-compat without silent loss; case-insensitive matching. (3) **7 more background jobs** now read alert windows from `TenantSettings` (the Doc/AssetWarranty/OverdueAsset/Training/Succession/Timesheet/Grievance jobs), using the `BackgroundJobSettingsHelper.ParseCsvDays` pattern from v13.3. (4) **Attendance corrections** — `CreateAttendanceCorrectionRequestCommandHandler` + both validators replaced `private const int MaxRetroactiveDays = 30` with the tenant-configured setting; validators use the existing `IValidationSettingsProvider` pattern (now exposes `AttendanceCorrectionMaxRetroactiveDays`). (5) **`EmployeeDocumentsController.expiringSoon`** filter now uses `max(DocumentExpiryAlertDaysCsv)` instead of fixed 30-day horizon. (6) EF migration `20260415184533_AddBusinessRulesConfigV13_4.cs` — additive only, all 9 columns with DB defaults. (7) `tests/TecAxle.Hrms.BusinessRules.Tests/` now **31 passing tests** (+13 new: 6 resolver + 7 defaults/guardrails). Payroll suite (27) still green. Strict re-sweep confirms zero `Role.Name ==` hits in production code, zero hardcoded alert-day arrays, zero `const int MaxRetroactiveDays`. See `HARDCODED_RULES_REMAINING_FIX.md`. Previous: v13.3 — Hardcoded Business Rules Fix. Moved 18 hardcoded business rules into tenant-aware, effective-date-aware configuration: (1) new `EndOfServicePolicy` + `EndOfServicePolicyTier` + `EndOfServiceResignationDeductionTier` entities replace the Saudi-hardcoded tier formula in `CalculateEndOfServiceCommandHandler`; `IEndOfServiceCalculator` service resolves policy by effective date + optional country, persists `AppliedPolicySnapshotJson` on `EndOfServiceBenefit`; Saudi default is auto-seeded per tenant (`TenantProvisioningService` + `SeedData.SeedDefaultEndOfServicePolicyAsync`) preserving pre-v13.3 behavior exactly. (2) `CreateLeaveEncashmentCommandHandler:91` and `CalculateFinalSettlementCommandHandler:52` replace `/ 30m` with `ITenantPayrollCalendarService.GetMonthlyDailyBasisAsync(branchId, date)` — honors `PayrollCalendarPolicy.BasisType`. (3) `ISystemUserResolver` eliminates the `AssignedByUserId = 1` FK risk in `DailyAttendanceGeneratorService` — resolves the tenant's `IsSystemUser=true` user with 5-min cache; throws on unresolvable. (4) **15 new `TenantSettings` fields** for password/login-lockout/probation/upload-cap/vacation & shift validation/excuse windows/OT config future cap/reminder-day CSVs for contract/visa/review/loan/frozen-workflow; 5 background jobs refactored to read them. (5) `LoginLockoutPolicy.ParseOrDefault` replaces the hardcoded 5/10/15 → 15m/1h/24h escalation ladder. (6) `IValidationSettingsProvider` integrates tenant thresholds into FluentValidation rule chains (vacation, bulk vacation, shift, excuse validators). (7) EOS CRUD API at `/api/v1/end-of-service-policies` with overlap guards on effective ranges + deletion guard when benefits reference the policy. (8) EF migration `20260415180127_AddHardcodedBusinessRulesConfigurationV13_3.cs` — additive (15 `TenantSettings` columns + 3 EOS tables + 2 `EndOfServiceBenefits` columns). (9) `tests/TecAxle.Hrms.BusinessRules.Tests/` 18 tests. See `HARDCODED_BUSINESS_RULES_FIX.md`. Previous: v13.2 — Module Entitlement Enforcement Hardening. Closed the HTTP-layer security gap where 22 controllers in paid modules (Recruitment, Onboarding, Performance, Offboarding, EmployeeLifecycle, Payroll) injected `IApplicationDbContext` directly and bypassed the MediatR `[RequiresModule]` pipeline — leaving ~180 endpoints reachable by tenants without the corresponding subscription. New `RequiresModuleEndpointAttribute` (`IAsyncAuthorizationFilter`) + companion `AllowModuleReadOnlyAttribute` mirror the MediatR contract at the HTTP layer. Applied class-level to all 22 unprotected controllers + `MobileScheduleController`; consolidated `ReportsController` onto the same attribute (replacing manual per-endpoint `IEntitlementService` calls); `PortalController` (5469 lines) now carries 63 per-method decorations across 17 modules. `UpdateSubscriptionPlanCommandHandler` now invalidates entitlement cache for every tenant on the updated plan. First real `[RequiresLimit]` usages: `CreateEmployeeCommand` (`MaxEmployees`) and `CreateBranchCommand` (`MaxBranches`). Bug fix: `ModuleEntitlementBehavior` and `UsageLimitBehavior` now resolve `Result<T>.Failure` correctly via `MakeGenericMethod` instead of throwing `UnauthorizedAccessException`. New `tests/TecAxle.Hrms.Entitlement.Tests/` project with 79 tests (filter unit + limit unit + reflection regression + WebApplicationFactory-based E2E HTTP) — all green. See `MODULE_ENTITLEMENT_ENFORCEMENT_FIX.md` for full per-controller coverage tables and remaining risks. Frontend unchanged. Previous: v13.1 — Payroll Production-Safety Follow-Ups Completed. Built on top of v13.0: (1) Full CRUD for `PayrollCalendarPolicy` via `/api/v1/payroll-settings/calendar-policies` + new 4th tab on the Payroll Settings admin page; (2) Payroll Run Audit browsing section on the period view page; (3) `PercentageOfGross` allowances now implemented via two-pass resolution (fixed + basic pct → overtime → then gross-based pct) with explicit basis noted on each detail line; (4) SystemAdmin-only `POST /api/v1/payroll-periods/{id}/admin-unlock` for retroactive correction of Paid periods — reverts to Processed, clears all lock fields, writes Adjustment audit with required reason, deliberately does not auto-recalc; (5) Fixed a bug where SI configuration's `AppliesToNationalityCode` field was being sent by the frontend but not persisted by the backend; (6) New xUnit test project `tests/TecAxle.Hrms.Payroll.Tests/` with 27 passing tests covering pure calculators (Proration, Tax, CalendarResolver, AbsenceDeduction). Previous: v13.0 Production-Safe Payroll Calculation Pipeline; v12.1 Grouped Sidebar Navigation; v12.0 Full Per-Tenant Database Isolation.
