...
export class CreateActionComponent implements OnInit {
  ...
  canEdit(): boolean {
    ...
    return !inEdit || this.permissionService.has('disciplinaryAction.update');
  }
  form!: FormGroup;
  saving = signal(false);
  ...
  actionTypes = ['VerbalWarning', 'WrittenWarning', 'FinalWarning', 'Suspension', 'Demotion', 'Termination', 'Probation', 'Other'];
  severities = ['Minor', 'Moderate', 'Major', 'Severe', 'Critical'];

  ngOnInit(): void {
    this.form = this.fb.group({
      employeeId: [null, Validators.required],
      actionType: ['VerbalWarning', Validators.required],
    ...
    this.service.getDisciplinaryAction(id).subscribe({
      next: (a) => {
        this.form.patchValue({
          ...a,
          incidentDate: a.incidentDate?.split('T')[0],
      ...
      error: () => { this.notification.error(this.i18n.t('common.error')); this.router.navigate(['/employee-relations/disciplinary-actions']); }
    });
  }
  ...
  onSubmit(): void {
    ...
    const obs$ = this.isEditMode()
      ? this.service.updateDisciplinaryAction(this.editId()!, val)
      : this.service.createDisciplinaryAction(val);
    (obs$ as any).subscribe({
      next: () => {
        this.notification.success(this.i18n.t(this.isEditMode() ? 'disciplinary_actions.updated' : 'disciplinary_actions.created'));
        this.router.navigate(['/employee-relations/disciplinary-actions']);
      },
