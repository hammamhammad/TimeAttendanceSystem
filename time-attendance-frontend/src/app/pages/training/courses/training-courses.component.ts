...
export class TrainingCoursesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    { key: 'code', label: this.i18n.t('training_courses.code'), sortable: true, priority: 'high' },
    { key: 'name', label: this.i18n.t('training_courses.name'), sortable: true, priority: 'high' },
    { key: 'categoryName', label: this.i18n.t('training_courses.category'), sortable: true, priority: 'medium' },
    { key: 'deliveryMethod', label: this.i18n.t('training_courses.delivery_method'), sortable: true, priority: 'medium' },
    { key: 'durationHours', label: this.i18n.t('training_courses.duration_hours'), sortable: true, priority: 'medium' },
  ...
  loadData(): void {
    ...
    this.service.getCourses({ search: this.searchTerm(), pageNumber: this.currentPage(), pageSize: this.pageSize() }).subscribe({
  ...
  navigateToCreate(): void { this.router.navigate(['/training/courses/create']); }
  ...
  async onDelete(item: TrainingCourseDto): Promise<void> {
    const result = await this.confirmation.confirm({
      title: this.i18n.t('training_courses.delete'), message: this.i18n.t('training_courses.confirm_delete'),
    ...
    if (result.confirmed) {
      this.service.deleteCourse(item.id).subscribe({
        next: () => { this.notification.success(this.i18n.t('training_courses.deleted')); this.loadData(); },
        error: () => this.notification.error(this.i18n.t('common.error'))
      });
    }
  }

  onActionClick(event: { action: string; item: TrainingCourseDto }): void {
    if (event.action === 'view') this.router.navigate(['/training/courses', event.item.id, 'view']);
    else if (event.action === 'edit') this.router.navigate(['/training/courses', event.item.id, 'edit']);
