...
import { TenantEntitlementSummary, SystemModule } from '../../shared/models/entitlement.model';
...
export class EntitlementService {
  ...
  /**
   * Check if a module is in read-only mode (disabled but was previously accessible).
   * Returns true when entitlements are loaded AND the module is NOT enabled.
   * Used to show read-only banners instead of blocking access entirely.
   */
  isModuleReadOnly(module: string): boolean {
    const ent = this.entitlements();
    if (!ent) return false; // not loaded yet, don't show read-only state
    return !ent.enabledModules.includes(module);
  }
