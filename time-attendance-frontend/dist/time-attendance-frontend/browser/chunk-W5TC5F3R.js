...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(CreateRoleComponent, [{
    type: Component,
    args: [{ selector: "app-create-role", standalone: true, imports: [CommonModule, FormsModule, RouterModule, FormHeaderComponent, FormSectionComponent], template: `<div class="container-fluid">\r
  <!-- Header -->\r
  <app-form-header\r
    mode="create"\r
    [title]="t('roles.create_role')"\r
    moduleName="roles"\r
    moduleRoute="roles"\r
...
\r
  @if (loading()) {\r
    ...
    <!-- Main Form -->\r
    <app-form-section [title]="t('roles.role_information')">\r
...
\r
        <form (ngSubmit)="onSubmit()">\r
          ...
          <app-form-section [title]="t('roles.basic_information')">\r
            <!-- Role Name -->\r
            <div class="row g-3">\r
              <div class="col-md-6">\r
                ...
                <input\r
                  ...
                  [(ngModel)]="roleForm.name"\r
                  name="roleName"\r
                  [placeholder]="t('roles.role_name_placeholder')"\r
...
\r
          ...
          <app-form-section [title]="t('roles.select_permissions')">\r
            <div class="d-flex justify-content-end align-items-center mb-3">\r
              <div class="btn-group btn-group-sm" role="group">\r
                <button \r
                  ...
                  [disabled]="submitting()"\r
                >\r
                  <i class="fa-solid fa-check-double me-1"></i>\r
                  {{ t('roles.select_all') }}\r
                </button>\r
                <button \r
                  type="button" \r
                  class="btn btn-outline-secondary"\r
                  (click)="clearAllPermissions()"\r
                  [disabled]="submitting()"\r
                >\r
...
\r
            <!-- Selected permissions summary -->\r
            <div class="alert alert-info py-2 px-3 mb-3">\r
              <small>\r
                <i class="fa-solid fa-info-circle me-2"></i>\r
                {{ roleForm.selectedPermissions.size }} {{ t('roles.permissions_selected') }} {{ t('common.of') }} {{ allPermissions().length }}\r
              </small>\r
            </div>\r
            \r
            <!-- Permissions search -->\r
            <div class="mb-3">\r
              <div class="input-group">\r
                <span class="input-group-text">\r
                  <i class="fa-solid fa-search"></i>\r
                </span>\r
                <input \r
                  type="text" \r
                  class="form-control" \r
                  [(ngModel)]="permissionSearchTerm"\r
                  name="permissionSearch"\r
                  [placeholder]="t('roles.search_permissions_placeholder')"\r
                  [disabled]="submitting()"\r
                />\r
              </div>\r
            </div>\r
            \r
            <!-- Permissions List -->\r
            <div class="permissions-list border rounded p-3" style="max-height: 500px; overflow-y: auto;">\r
              @for (group of getFilteredGroupedPermissions(); track group.group) {\r
                <div class="permission-group mb-4">\r
                  <!-- Group Header with Actions -->\r
                  <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">\r
                    <div class="d-flex align-items-center">\r
                      <!-- Collapse/Expand Button -->\r
                      <button\r
                        type="button"\r
                        class="btn btn-sm btn-link text-decoration-none p-0 me-2"\r
                        (click)="toggleGroupCollapse(group.group)"\r
                        [disabled]="submitting()"\r
                        [title]="isGroupCollapsed(group.group) ? t('common.expand') : t('common.collapse')">\r
                        <i class="fas" [ngClass]="isGroupCollapsed(group.group) ? 'fa-chevron-right' : 'fa-chevron-down'"></i>\r
                      </button>\r
\r
                      <h6 class="fw-bold text-primary mb-0">\r
                        <i class="fas fa-layer-group me-2"></i>\r
                        {{ group.group }}\r
                        <small class="text-muted fw-normal">({{ group.permissions.length }} {{ t('roles.permissions') }})</small>\r
                      </h6>\r
                    </div>\r
\r
                    <!-- Group Actions -->\r
                    <div class="btn-group btn-group-sm" role="group">\r
                      <button\r
                        type="button"\r
                        class="btn"\r
                        [class.btn-primary]="areAllGroupPermissionsSelected(group)"\r
                        [class.btn-outline-primary]="!areAllGroupPermissionsSelected(group)"\r
                        (click)="toggleGroupSelection(group)"\r
                        [disabled]="submitting()"\r
                        [title]="areAllGroupPermissionsSelected(group) ? t('roles.deselect_all_group') : t('roles.select_all_group')">\r
                        <i class="fas"\r
                           [ngClass]="areAllGroupPermissionsSelected(group) ? 'fa-check-square' : (areSomeGroupPermissionsSelected(group) ? 'fa-minus-square' : 'fa-square')"></i>\r
                        {{ areAllGroupPermissionsSelected(group) ? t('roles.deselect_all') : t('roles.select_all') }}\r
                      </button>\r
                    </div>\r
                  </div>\r
\r
                  <!-- Permissions Grid (Collapsible) -->\r
                  @if (!isGroupCollapsed(group.group)) {\r
                    <div class="row g-3">\r
                    @for (permission of group.permissions; track permission.id) {\r
                      <div class="col-md-6 col-lg-4">\r
                        <div class="permission-item border rounded p-3" \r
                             [class.bg-primary-subtle]="isPermissionSelectedForRole(permission.id)"\r
                             [class.border-primary]="isPermissionSelectedForRole(permission.id)">\r
                          <div class="form-check d-flex align-items-start">\r
                            <input\r
                              class="form-check-input mt-1"\r
                              type="checkbox"\r
                              [id]="'permission-' + permission.id"\r
                              [checked]="isPermissionSelectedForRole(permission.id)"\r
                              [disabled]="submitting()"\r
                              (change)="onTogglePermissionForRole(permission)"\r
                            />\r
                            <div class="form-check-content ms-2 flex-grow-1">\r
                              <div class="d-flex align-items-center mb-1">\r
                                <i class="fas me-2 text-muted" [ngClass]="getPermissionIcon(permission.key)" style="font-size: 0.8rem;"></i>\r
                                ...
                                <span class="badge" [ngClass]="getActionBadgeClass(permission.key)" style="font-size: 0.65rem;">\r
                                  {{ getPermissionAction(permission.key) }}\r
                                </span>\r
                              </div>\r
                              <p class="text-muted small mb-0" style="font-size: 0.75rem; margin-left: 1.5rem;">\r
              ...
              \r
              @if (getFilteredGroupedPermissions().length === 0) {\r
                <div class="text-center py-4">\r
                  <i class="fa-solid fa-key fa-2x text-muted mb-2"></i>\r
                  <p class="text-muted mb-0">{{ t('roles.no_permissions_available') }}</p>\r
                </div>\r
              }\r
            </div>\r
          </app-form-section>\r
\r
          ...
          <div class="d-flex justify-content-end gap-2">\r
            ...
            <button \r
              ...
              [disabled]="!roleForm.name.trim() || submitting()">\r
              @if (submitting()) {\r
                <div class="spinner-border spinner-border-sm me-2"></div>\r
              } @else {\r
                <i class="fa-solid fa-save me-2"></i>\r
              }\r
              {{ submitting() ? t('roles.creating_role') : t('roles.create_role') }}\r
            </button>\r
          </div>\r
        </form>\r
      </div>\r
...
//# sourceMappingURL=chunk-W5TC5F3R.js.map
