...
var _DailyAttendanceComponent = class _DailyAttendanceComponent {
  ...
  presentEmployees = computed(() => this.filteredRecords().filter((r) => r.status === AttendanceStatus.Present).length, ...ngDevMode ? [{ debugName: "presentEmployees" }] : []);
  absentEmployees = computed(() => this.filteredRecords().filter((r) => r.status === AttendanceStatus.Absent).length, ...ngDevMode ? [{ debugName: "absentEmployees" }] : []);
  ...
  getAttendanceRatePercentage() {
    ...
    const presentRecords = records.filter((r) => r.status === AttendanceStatus.Present || r.status === AttendanceStatus.Late || r.status === AttendanceStatus.Overtime).length;
  ...
  hasIncompleteRecords() {
    return this.filteredRecords().some((record) => record.status === AttendanceStatus.Incomplete || record.status === AttendanceStatus.Pending);
  }
  /**
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DailyAttendanceComponent, [{
    type: Component,
    args: [{ selector: "app-daily-attendance", standalone: true, imports: [CommonModule, FormsModule, RouterModule, SearchableSelectComponent, DataTableComponent, PageHeaderComponent, LoadingSpinnerComponent], template: `<div class="container-fluid p-4">
  <!-- Page Header -->
  <app-page-header
...
//# sourceMappingURL=chunk-4R75JKZS.js.map
