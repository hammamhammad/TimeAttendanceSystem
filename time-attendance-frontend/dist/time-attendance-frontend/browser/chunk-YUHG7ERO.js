...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ShiftsComponent, [{
    type: Component,
    args: [{ selector: "app-shifts", standalone: true, imports: [FormsModule, HasPermissionDirective, SearchableSelectComponent, DataTableComponent, PageHeaderComponent, UnifiedFilterComponent, StatusBadgeComponent, ModalWrapperComponent], template: `<div class="app-list-page">\r
  <!-- Page Header -->\r
  <app-page-header [title]="t('shifts.title')">\r
  </app-page-header>\r
\r
  <!-- Standardized Filters Component -->\r
  <app-unified-filter moduleName="shifts" [refreshing]="loading()" (searchChange)="onSearchChange($event)"\r
    (filtersChange)="onFiltersChange($event)" (add)="onCreateShift()" (refresh)="onRefreshData()">\r
  </app-unified-filter>\r
\r
  <!-- Shifts Table -->\r
\r
  <app-data-table [data]="tableData() || []" [columns]="tableColumns()" [actions]="tableActions()"\r
    [currentPage]="currentPage" [totalPages]="totalPages" [totalItems]="totalCount" [pageSize]="pageSize"\r
    [loading]="loading" [emptyTitle]="t('shifts.noShifts')" [emptyMessage]="t('shifts.noShiftsDescription')"\r
    [responsiveMode]="'auto'" (actionClick)="onActionClick($event)" (pageChange)="onTablePageChange($event)"\r
...
\r
<!-- Create/Edit Modal -->\r
<app-modal-wrapper [show]="showCreateModal() || showEditModal()"\r
  [title]="showCreateModal() ? t('shifts.actions.create') : t('shifts.actions.edit')" [size]="'lg'" [centered]="true"\r
  [loading]="submitting()" (close)="onCloseModal()">\r
\r
  <div class="modal-body">\r
    <form>\r
      <!-- Basic Information -->\r
      <div class="mb-3">\r
        <label class="form-label required">{{ t('shifts.fields.name') }}</label>\r
        <input type="text" class="form-control" [(ngModel)]="shiftForm.name" name="name" required>\r
      </div>\r
\r
      <div class="mb-3">\r
        <label class="form-label">{{ t('shifts.fields.description') }}</label>\r
        <textarea class="form-control" rows="2" [(ngModel)]="shiftForm.description" name="description"></textarea>\r
      </div>\r
\r
      <!-- Status -->\r
      <div class="mb-3">\r
        <label class="form-label required">{{ t('shifts.fields.status') }}</label>\r
        <app-searchable-select [options]="statusSelectOptions" [(ngModel)]="shiftForm.status" name="status"\r
          [placeholder]="t('common.select')" [required]="true" [searchable]="true"\r
          [clearable]="false"></app-searchable-select>\r
      </div>\r
\r
      <!-- Shift Type -->\r
      <div class="mb-3">\r
        <label class="form-label required">{{ t('shifts.fields.type') }}</label>\r
        <div class="row">\r
          <div class="col-md-6">\r
            <div class="form-check">\r
              <input class="form-check-input" type="radio" name="shiftType" [value]="ShiftType.TimeBased"\r
                [(ngModel)]="shiftForm.shiftType" (change)="onShiftTypeChange()">\r
              <label class="form-check-label">\r
                {{ t('shifts.types.timeBased') }}\r
              </label>\r
              <small class="form-text text-muted d-block">{{ t('shifts.types.timeBasedDescription') }}</small>\r
            </div>\r
          </div>\r
          <div class="col-md-6">\r
            <div class="form-check">\r
              <input class="form-check-input" type="radio" name="shiftType" [value]="ShiftType.HoursOnly"\r
                [(ngModel)]="shiftForm.shiftType" (change)="onShiftTypeChange()">\r
              <label class="form-check-label">\r
                {{ t('shifts.types.hoursOnly') }}\r
              </label>\r
              <small class="form-text text-muted d-block">{{ t('shifts.types.hoursOnlyDescription') }}</small>\r
            </div>\r
          </div>\r
        </div>\r
      </div>\r
\r
      <!-- Hours Only Configuration -->\r
      @if (shiftForm.shiftType === ShiftType.HoursOnly) {\r
      <div class="mb-3">\r
        <label class="form-label required">{{ t('shifts.fields.requiredHours') }}</label>\r
        <div class="input-group">\r
          <input type="number" class="form-control" [(ngModel)]="shiftForm.requiredHours" name="requiredHours" min="0.5"\r
            max="24" step="0.5">\r
...
\r
      <!-- Time-Based Configuration -->\r
      @if (shiftForm.shiftType === ShiftType.TimeBased) {\r
      <div>\r
        <!-- Shift Periods -->\r
        <div class="mb-3">\r
          <div class="d-flex justify-content-between align-items-center mb-2">\r
            <label class="form-label required">{{ t('shifts.fields.periods') }}</label>\r
            <button type="button" class="btn btn-sm btn-outline-primary" (click)="addShiftPeriod()"\r
              [disabled]="shiftForm.shiftPeriods.length >= 2">\r
              <i class="fas fa-plus me-1"></i>{{ t('shifts.addPeriod') }}\r
            </button>\r
          </div>\r
\r
          @for (period of shiftForm.shiftPeriods; track i; let i = $index) {\r
          <div class="card mb-2">\r
            <div class="card-body p-3">\r
              <div class="d-flex justify-content-between align-items-center mb-2">\r
                <h6 class="mb-0">{{ t('shifts.period') }} {{ period.periodOrder }}</h6>\r
                <button type="button" class="btn btn-sm btn-outline-danger" (click)="removeShiftPeriod(i)"\r
                  [disabled]="shiftForm.shiftPeriods.length <= 1">\r
                  <i class="fas fa-trash"></i>\r
                </button>\r
              </div>\r
              <div class="row">\r
                <div class="col-md-6">\r
                  <label class="form-label">{{ t('shifts.fields.startTime') }}</label>\r
                  <input type="time" class="form-control" [(ngModel)]="period.startTime" [name]="'startTime' + i"\r
                    required>\r
                </div>\r
                <div class="col-md-6">\r
                  <label class="form-label">{{ t('shifts.fields.endTime') }}</label>\r
                  <input type="time" class="form-control" [(ngModel)]="period.endTime" [name]="'endTime' + i" required>\r
                </div>\r
              </div>\r
            </div>\r
          </div>\r
...
\r
        ...
        <div class="mb-3">\r
          <div class="form-check mb-2">\r
            <input class="form-check-input" type="checkbox" [(ngModel)]="shiftForm.allowFlexibleHours"\r
              name="allowFlexibleHours" (change)="onFlexibleHoursChange()">\r
            <label class="form-check-label">\r
              {{ t('shifts.fields.allowFlexibleHours') }}\r
            </label>\r
            <small class="form-text text-muted d-block">{{ t('shifts.flexibleHoursDescription') }}</small>\r
          </div>\r
\r
          @if (shiftForm.allowFlexibleHours) {\r
          <div class="row">\r
            <div class="col-md-6">\r
              <label class="form-label">{{ t('shifts.fields.flexMinutesBefore') }}</label>\r
              <div class="input-group">\r
                <input type="number" class="form-control" [(ngModel)]="shiftForm.flexMinutesBefore"\r
                  name="flexMinutesBefore" min="1" max="480">\r
                <span class="input-group-text">{{ t('common.minutes') }}</span>\r
              </div>\r
            </div>\r
            <div class="col-md-6">\r
              <label class="form-label">{{ t('shifts.fields.flexMinutesAfter') }}</label>\r
              <div class="input-group">\r
                <input type="number" class="form-control" [(ngModel)]="shiftForm.flexMinutesAfter"\r
                  name="flexMinutesAfter" min="1" max="480">\r
                <span class="input-group-text">{{ t('common.minutes') }}</span>\r
              </div>\r
            </div>\r
          </div>\r
...
\r
        <!-- Grace Period -->\r
        <div class="mb-3">\r
          <label class="form-label">{{ t('shifts.fields.gracePeriod') }}</label>\r
          <div class="input-group">\r
            <input type="number" class="form-control" [(ngModel)]="shiftForm.gracePeriodMinutes"\r
              name="gracePeriodMinutes" min="1" max="120">\r
            <span class="input-group-text">{{ t('common.minutes') }}</span>\r
          </div>\r
          <small class="form-text text-muted">{{ t('shifts.gracePeriodDescription') }}</small>\r
          @if (shiftForm.allowFlexibleHours && shiftForm.gracePeriodMinutes) {\r
          <small class="form-text text-info d-block">\r
            <i class="fas fa-info-circle me-1"></i>{{ t('shifts.validation.flexibleHoursAndGraceCombined') }}\r
          </small>\r
          }\r
        </div>\r
      </div>\r
...
\r
      <!-- Check-in/Check-out Options -->\r
      <div class="row mb-3">\r
        <div class="col-md-6">\r
          <div class="form-check">\r
            <input class="form-check-input" type="checkbox" [(ngModel)]="shiftForm.isCheckInRequired"\r
              name="isCheckInRequired">\r
            <label class="form-check-label">\r
              {{ t('shifts.fields.checkInRequired') }}\r
            </label>\r
            <small class="form-text text-muted d-block">{{ t('shifts.checkInRequiredDescription') }}</small>\r
          </div>\r
        </div>\r
        <div class="col-md-6">\r
          <div class="form-check">\r
            <input class="form-check-input" type="checkbox" [(ngModel)]="shiftForm.isAutoCheckOut"\r
              name="isAutoCheckOut">\r
            <label class="form-check-label">\r
              {{ t('shifts.fields.autoCheckOut') }}\r
            </label>\r
            <small class="form-text text-muted d-block">{{ t('shifts.autoCheckOutDescription') }}</small>\r
          </div>\r
        </div>\r
      </div>\r
    </form>\r
...
\r
  <div modal-footer class="d-flex gap-2 justify-content-end">\r
    <button type="button" class="btn btn-secondary" (click)="onCloseModal()">\r
      {{ t('common.cancel') }}\r
    </button>\r
    <button type="button" class="btn btn-primary" (click)="showCreateModal() ? onSubmitCreate() : onSubmitEdit()"\r
      [disabled]="!isFormValid() || submitting()">\r
      @if (submitting()) {\r
      <span class="spinner-border spinner-border-sm me-2"></span>\r
      }\r
      {{ showCreateModal() ? t('common.create') : t('common.update') }}\r
    </button>\r
  </div>\r
\r
</app-modal-wrapper>\r
...
<app-modal-wrapper [show]="showDeleteModal()" [title]="t('shifts.actions.delete')" [centered]="true"\r
  [loading]="submitting()" (close)="onCloseModal()">\r
\r
  <div class="modal-body">\r
    <p>{{ t('shifts.deleteConfirmation') }}</p>\r
    @if (selectedShift()) {\r
    <p><strong>{{ selectedShift()?.name }}</strong></p>\r
    }\r
  </div>\r
\r
  <div modal-footer class="d-flex gap-2 justify-content-end">\r
    ...
    <button type="button" class="btn btn-danger" (click)="onConfirmDelete()" [disabled]="submitting()">\r
      @if (submitting()) {\r
      <span class="spinner-border spinner-border-sm me-2"></span>\r
      }\r
      {{ t('common.delete') }}\r
...
\r
<!-- Default Shift Confirmation Modal -->\r
<app-modal-wrapper [show]="showDefaultShiftModal()" [title]="t('shifts.defaultShift.confirmReplace')" [centered]="true"\r
  [loading]="submitting()" (close)="onCloseModal()">\r
\r
  <div class="modal-body">\r
    <p>{{ t('shifts.defaultShift.confirmReplaceMessage') }}</p>\r
    <p><strong>{{ t('shifts.defaultShift.newShift') }}:</strong> {{ selectedShift()?.name }}</p>\r
    <p><strong>{{ t('shifts.defaultShift.currentShift') }}:</strong> {{ defaultShift()?.name }}</p>\r
  </div>\r
\r
  <div modal-footer class="d-flex gap-2 justify-content-end">\r
    <button type="button" class="btn btn-secondary" (click)="onCloseModal()">\r
      {{ t('shifts.defaultShift.cancelButton') }}\r
    </button>\r
    <button type="button" class="btn btn-success" (click)="onConfirmSetDefaultShift()" [disabled]="submitting()">\r
      @if (submitting()) {\r
      <span class="spinner-border spinner-border-sm me-2"></span>\r
      }\r
      {{ t('shifts.defaultShift.confirmButton') }}\r
    </button>\r
  </div>\r
\r
...
//# sourceMappingURL=chunk-YUHG7ERO.js.map
