...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(UserFormComponent, [{
    type: Component,
    args: [{ selector: "app-user-form", standalone: true, imports: [ReactiveFormsModule, RouterModule, FormSectionComponent, SearchableSelectComponent], template: `<div class="container-fluid">\r
  <!-- Header -->\r
      ...
      <nav aria-label="breadcrumb">\r
        <ol class="breadcrumb">\r
          <li class="breadcrumb-item">\r
            <a routerLink="/dashboard">{{ t('dashboard.title') }}</a>\r
          </li>\r
          <li class="breadcrumb-item">\r
            <a routerLink="/users">{{ t('users.title') }}</a>\r
          </li>\r
          <li class="breadcrumb-item active">{{ modalTitle() }}</li>\r
        </ol>\r
      </nav>\r
    </div>\r
    <button \r
      ...
      (click)="closeModal()"\r
      [disabled]="loading()">\r
      <i class="fa-solid fa-arrow-left me-2"></i>\r
      {{ t('common.back') }}\r
    </button>\r
  </div>\r
\r
  <!-- Main Form Card -->\r
  <app-form-section [title]="t('users.user_information')">\r
    <div class="card-body">\r
      <form [formGroup]="userForm" (ngSubmit)="onSubmit()">\r
            @if (error()) {\r
...
\r
            <div class="row g-3">\r
              <!-- Employee Selection (only for create and not for SystemAdmin) -->\r
              @if (!isEditMode() && !isSystemAdmin()) {\r
                <div class="col-md-12">\r
                  <label class="form-label">{{ t('users.employee') }} <span class="text-danger">*</span></label>\r
                  <app-searchable-select\r
                    [options]="employeeOptions()"\r
                    [value]="userForm.get('employeeId')?.value"\r
                    [placeholder]="t('common.select')"\r
                    (selectionChange)="onEmployeeSelected($event); userForm.get('employeeId')?.setValue($event)"\r
                  ></app-searchable-select>\r
                  @if (isFieldInvalid('employeeId')) {\r
                    <div class="text-danger small mt-1">\r
                      <i class="fa-solid fa-exclamation-triangle me-1"></i>\r
                      {{ getFieldError('employeeId') }}\r
                    </div>\r
                  }\r
                  <small class="form-text text-muted">\r
                    <i class="fa-solid fa-info-circle me-1"></i>\r
                    {{ t('users.employee_help') }}\r
                  </small>\r
                </div>\r
              }\r
\r
              <!-- Username (only for create) - Auto-filled from employee -->\r
              @if (!isEditMode()) {\r
                <div class="col-md-6">\r
                  <label class="form-label">{{ t('users.username') }} <span class="text-danger">*</span></label>\r
                  <input\r
                    type="text"\r
                    class="form-control"\r
                    formControlName="username"\r
                    [placeholder]="t('users.username')"\r
                    [class.is-invalid]="isFieldInvalid('username')"\r
                    readonly\r
                  />\r
                  @if (isFieldInvalid('username')) {\r
                    <div class="invalid-feedback">{{ getFieldError('username') }}</div>\r
                  }\r
                </div>\r
              }\r
\r
              <!-- Email - Auto-filled from employee in create mode -->\r
              <div class="col-md-6">\r
                <label class="form-label">{{ t('users.email') }} <span class="text-danger">*</span></label>\r
                <input\r
                  type="email"\r
                  class="form-control"\r
                  formControlName="email"\r
                  [placeholder]="t('users.email')"\r
                  [class.is-invalid]="isFieldInvalid('email')"\r
                  [readonly]="!isEditMode()"\r
                />\r
                @if (isFieldInvalid('email')) {\r
                  <div class="invalid-feedback">{{ getFieldError('email') }}</div>\r
                }\r
              </div>\r
\r
              <!-- Phone -->\r
              <div class="col-md-6">\r
                <label class="form-label">{{ t('users.phone') }}</label>\r
                <input\r
                  type="tel"\r
                  class="form-control"\r
                  formControlName="phone"\r
                  [placeholder]="t('users.phone')"\r
                  [class.is-invalid]="isFieldInvalid('phone')"\r
                />\r
                @if (isFieldInvalid('phone')) {\r
                  <div class="invalid-feedback">{{ getFieldError('phone') }}</div>\r
                }\r
              </div>\r
\r
              <!-- Preferred Language -->\r
              <div class="col-md-6">\r
                <label class="form-label">{{ t('users.language') }} <span class="text-danger">*</span></label>\r
                <select\r
                  class="form-select"\r
                  formControlName="preferredLanguage"\r
                  [class.is-invalid]="isFieldInvalid('preferredLanguage')"\r
                >\r
                  <option value="en">{{ t('common.language_english') }}</option>\r
                  <option value="ar">{{ t('common.language_arabic') }}</option>\r
                </select>\r
                @if (isFieldInvalid('preferredLanguage')) {\r
                  <div class="invalid-feedback">{{ getFieldError('preferredLanguage') }}</div>\r
                }\r
              </div>\r
\r
              <!-- Password Row (only for create) -->\r
              @if (!isEditMode()) {\r
                <!-- Password -->\r
                <div class="col-md-6">\r
                  <label class="form-label">{{ t('users.password') }} <span class="text-danger">*</span></label>\r
                  <input\r
                    type="password"\r
                    class="form-control"\r
                    formControlName="password"\r
                    [placeholder]="t('users.password')"\r
                    [class.is-invalid]="isFieldInvalid('password')"\r
                  />\r
                  @if (isFieldInvalid('password')) {\r
                    <div class="invalid-feedback">{{ getFieldError('password') }}</div>\r
                  }\r
                  <small class="form-text text-muted">\r
                    {{ t('auth.password_requirements') }}\r
                  </small>\r
                </div>\r
\r
                <!-- Confirm Password -->\r
                <div class="col-md-6">\r
                  <label class="form-label">{{ t('users.confirm_password') }} <span class="text-danger">*</span></label>\r
                  <input\r
                    type="password"\r
                    class="form-control"\r
                    formControlName="confirmPassword"\r
                    [placeholder]="t('users.confirm_password')"\r
                    [class.is-invalid]="isFieldInvalid('confirmPassword')"\r
                  />\r
                  @if (isFieldInvalid('confirmPassword')) {\r
                    <div class="invalid-feedback">{{ getFieldError('confirmPassword') }}</div>\r
                  }\r
                </div>\r
              }\r
\r
              <!-- Status (only for edit) -->\r
              @if (isEditMode()) {\r
                <div class="col-md-6">\r
                  <label class="form-label">{{ t('common.status') }}</label>\r
...
\r
              <!-- Must Change Password -->\r
              <div class="col-md-6">\r
                <label class="form-label">{{ t('users.password_status') }}</label>\r
                <div class="form-check form-switch mt-2">\r
                  <input\r
                    class="form-check-input"\r
                    type="checkbox"\r
                    role="switch"\r
                    id="mustChangePasswordSwitch"\r
                    formControlName="mustChangePassword"\r
                  />\r
                  <label class="form-check-label" for="mustChangePasswordSwitch">\r
                    {{ t('users.must_change_password') }}\r
                  </label>\r
                </div>\r
                <small class="form-text text-muted">\r
                  {{ t('users.must_change_password_hint') }}\r
                </small>\r
              </div>\r
            </div>\r
\r
            <!-- Permissions Section -->\r
              <app-form-section [title]="t('users.permissions')">\r
                <!-- Roles -->\r
                <div class="mb-3">\r
                  <label class="form-label">{{ t('users.roles') }} <span class="text-danger">*</span></label>\r
                  <div class="border rounded p-3 bg-light">\r
                    @if (availableRoles().length > 0) {\r
                      <div class="row g-3">\r
                        @for (role of availableRoles(); track role.id) {\r
                          <div class="col-md-6 col-lg-4">\r
                            <div class="form-check p-2 border rounded bg-white">\r
                              <input\r
                                class="form-check-input"\r
                                type="checkbox"\r
                                [id]="'role-' + role.id"\r
                                [checked]="isRoleSelected(role.id)"\r
                                (change)="onRoleToggle(role.id)"\r
                              />\r
                              <label class="form-check-label fw-medium" [for]="'role-' + role.id">\r
                                <i class="fa-solid fa-shield-alt me-2 text-primary"></i>\r
                                {{ role.name }}\r
                              </label>\r
                            </div>\r
                          </div>\r
                      ...
                      <div class="text-muted text-center py-3">\r
                        <i class="fa-solid fa-spinner fa-spin me-2"></i>\r
                        {{ t('common.loading') }}...\r
                      </div>\r
                    }\r
                    @if (isFieldInvalid('roleIds')) {\r
                      <div class="text-danger small mt-2">\r
                        <i class="fa-solid fa-exclamation-triangle me-1"></i>\r
                        {{ getFieldError('roleIds') }}\r
                      </div>\r
                    }\r
                  </div>\r
                </div>\r
\r
                <!-- Branches -->\r
                <div class="mb-3">\r
                  <label class="form-label">{{ t('users.branches') }}</label>\r
                  <div class="border rounded p-3 bg-light">\r
                    @if (availableBranches().length > 0) {\r
                      <div class="row g-3">\r
                          ...
                          <div class="col-md-6 col-lg-4">\r
                            <div class="form-check p-2 border rounded bg-white">\r
                              <input\r
                                class="form-check-input"\r
                                type="checkbox"\r
                                [id]="'branch-' + branch.id"\r
                                [checked]="isBranchSelected(branch.id)"\r
                                (change)="onBranchToggle(branch.id)"\r
                              />\r
                              <label class="form-check-label fw-medium" [for]="'branch-' + branch.id">\r
                                <i class="fa-solid fa-building me-2 text-success"></i>\r
                                {{ branch.name }}\r
                              </label>\r
                            </div>\r
                          </div>\r
                    ...
                    <small class="form-text text-muted d-block mt-3 px-2">\r
                      <i class="fa-solid fa-info-circle me-1 text-info"></i>\r
                      {{ t('users.branches_help') }}\r
                    </small>\r
                  </div>\r
                </div>\r
              </app-form-section>\r
\r
        ...
        <div class="d-flex justify-content-end gap-2 mt-4">\r
          <button type="button" class="btn btn-outline-secondary" (click)="closeModal()" [disabled]="loading()">\r
          ...
          <button \r
            ...
            [disabled]="userForm.invalid || loading()"\r
          >\r
            @if (loading()) {\r
            ...
            @if (isEditMode()) {\r
              ...
              <i class="fa-solid fa-plus me-2"></i>\r
              {{ t('users.create_user') }}\r
            }\r
          </button>\r
        </div>\r
      </form>\r
...
//# sourceMappingURL=chunk-YCW64LTP.js.map
